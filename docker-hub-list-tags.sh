	...
	cat <<-EOUSAGE
		...
		usage: $self [options] repo [repo ...]
		   ...
		   ie: $self library/docker
		       ...
		       $self library/docker docker-pkg/docker-master | sort -uV

		       $self --jq '.results[]' docker-pkg/syncthing

		       $self --jq '.results[] | [ .name, (.full_size | tostring), .last_updated ] | join("\\t")' docker-pkg/syncthing | sort -V | column -ts \$'\\t'
